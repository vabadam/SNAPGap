...
df <- df %>% 
  mutate(
    variable_name = "number_food_hubs", 
...
df <- df %>% 
  ...
  mutate(
    variable_name = "number_meat_processors")
...
## Number of operations per county
df_county <- df %>% 
  group_by(fips, state_name, county_name, variable_name) %>% 
  summarise(value = sum(value)) 
