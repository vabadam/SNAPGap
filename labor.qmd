...
Additionally, we use [2013 Rural-Urban Continuum Codes from the USDA Economic Research Service](https://www.ers.usda.gov/data-products/rural-urban-continuum-codes/) to better understand the rural-urban locations of these counties. The rural urban continuum code (RUCC) was used to divide producers into three categories of ruralness: metro (RUCC 7-9), metro-adjacent (RUCC 4-6), and nonmetro (RUCC 1-3) to account for the degree of urbanization and adjacency to a metro area.
...
meta_qcew <- meta_qcew %>% 
  mutate(
    variable_definition = case_when(
      ...
      str_detect(variable_name, "annual_avg_emplvl") ~ "Annual average of monthly employment levels for the specific NACIS sector for a given year", 
      ...
      str_detect(variable_name, "annual_avg_estabs_count") ~ "Annual average of monthly employment levels for the specific NACIS sector for a given year"))
  ...
  mutate(
    ...
    variable_definition = case_when(
      ...
      variable_name == "farming_fishing_and_forestry_occupations_male" ~ 
        "Number of males in farming fishing and foresty occupations for the civilian employed population, 16 years and over",
...
meta_farm_labor <- tibble (
  ...
  user_friendly_variable_name = c("Labor, hired, number of workers", 
                          ...
                          "Labor, unpaind, number of workers"),
  category = "Labor",
  topic_area = "Employment", 
